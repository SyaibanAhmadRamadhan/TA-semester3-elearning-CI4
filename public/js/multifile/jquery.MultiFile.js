...
if (window.jQuery)
    (function ($) {
        ...
        function FILE_LIST(x) {
            return (
                (x.files && x.files.length ? x.files : null) || [
                    {
                        name: x.value,
                        size: 0,
        ...
        $.fn.MultiFile = function (options) {
            ...
            if (typeof arguments[0] == "string") {
                ...
                return $.fn.MultiFile[arguments[0]].apply(
                    this,
            ...
            if ($.fn.MultiFile.options.autoIntercept) {
                $.fn.MultiFile.intercept(
                    $.fn.MultiFile.options
                        .autoIntercept /* array of methods to intercept */
                );
                $.fn.MultiFile.options.autoIntercept =
            ...
            this.not(".MultiFile-applied")
                ...
                .each(function () {
                    ...
                    if (!(o.max > 0) /*IsNull(MultiFile.max)*/) {
                        o.max = (String(
                            MultiFile.e.className.match(
                                /\b(max|limit)\-([0-9]+)\b/gi
                            ) || [""]
                        ).match(/[0-9]+/gi) || [""])[0];
                        if (!(o.max > 0)) o.max = -1;
                    ...
                    if (!o.accept) {
                        ...
                        o.accept = new String(o.accept).replace(
                            /^(accept|ext)\-/i,
                            ""
                        );
                    }
                    ...
                    o.maxsize =
                        o.maxsize > 0
                            ? o.maxsize
                            : null || MultiFile.E.data("maxsize") || 0;
                    ...
                    o.maxfile =
                        o.maxfile > 0
                            ? o.maxfile
                            : null || MultiFile.E.data("maxfile") || 0;
                    if (!(o.maxfile > 0) /*IsNull(MultiFile.maxfile)*/) {
                        o.maxfile = (String(
                            MultiFile.e.className.match(
                                /\b(maxfile|filemax)\-([0-9]+)\b/gi
                            ) || [""]
                        ).match(/[0-9]+/gi) || [""])[0];
                    ...
                    if (o.multiple !== false) {
                        if (o.max > 1)
                            MultiFile.E.attr("multiple", "multiple").prop(
                                "multiple",
                                true
                            );
                    }
                    ...
                    $.extend(MultiFile, {
                        ...
                        files: [],
                        instanceKey:
                            MultiFile.e.id || "MultiFile" + String(group_count), // Instance Key?
                        generateID: function (z) {
                            return (
                                MultiFile.instanceKey +
                                (z > 0 ? "_F" + String(z) : "")
                            );
                        },
                        trigger: function (event, element, MultiFile, files) {
                            var rv,
                                handler =
                                    MultiFile[event] || MultiFile["on" + event];
                            if (handler) {
                                files =
                                    files || MultiFile.files || FILE_LIST(this);
                                $.each(files, function (i, file) {
                                    // execute function in element's context, so 'this' variable is current element
                                    rv = handler.apply(MultiFile.wrapper, [
                                        element,
                                        file.name,
                                        MultiFile,
                                        file,
                    ...
                    MultiFile.e.name =
                        MultiFile.e.name || "file" + group_count + "[]";
                    ...
                    MultiFile.addSlave = function (slave, slave_count) {
                        ...
                        if (
                            MultiFile.max > 0 &&
                            MultiFile.files.length > MultiFile.max
                        ) {
                            slave.disabled = true;
                            disable_slave = true;
                        }
                        ...
                        slave.change(function (a, b, c) {
                            ...
                            $.each(files, function (i, file) {
                                ...
                                if (
                                    MultiFile.accept &&
                                    v &&
                                    !v.match(MultiFile.rxAccept)
                                ) {
                                    ERROR[ERROR.length] = p(
                                        MultiFile.STRING.denied
                                    );
                                    MultiFile.trigger(
                                        "FileInvalid",
                                        this,
                                ...
                                // check extension
                                var customError = MultiFile.trigger(
                                    "FileValidate",
                                    this,
                                    MultiFile,
                                    [file]
                            ...
                            if (
                                MultiFile.max > 0 &&
                                prevs.length + files.length > MultiFile.max
                            ) {
                                ERROR[ERROR.length] =
                                    MultiFile.STRING.toomany.replace(
                                        "$max",
                                        MultiFile.max
                                    );
                                MultiFile.trigger(
                            ...
                            if (
                                MultiFile.maxsize > 0 &&
                                total_size > MultiFile.maxsize
                            ) {
                                ERROR[ERROR.length] =
                                    MultiFile.STRING.toomuch.replace(
                                        "$size",
                                        sl(total_size) +
                                            " > " +
                                            sl(MultiFile.maxsize)
                                    );
                                MultiFile.trigger(
                                    "FileTooMuch",
                                    this,
                    ...
                    MultiFile.addToList = function (slave, slave_count, files) {
                        ...
                        var names = $("<span/>");
                        $.each(files, function (i, file) {
                            var v = String(file.name || "").replace(
                                    /[&<>'"]/g,
                                    function (c) {
                                        return "&#" + c.charCodeAt() + ";";
                                    }
                            ...
                            if (i > 0 && !!o.separator)
                                names.append(o.separator);
                            names.append(label);

                            var v = String(file.name || "");
                            names[names.length] = (
                                '<span class="MultiFile-title" title="' +
                                MultiFile.STRING.selected +
                                '">' +
                        ...
                        // Insert label
                        MultiFile.list.append(r.append(b, " ", names));
	...
	*/
        $.extend($.fn.MultiFile, {
            ...
            data: function () {
                ...
                if (!wp || !wp.length)
                    return !console.error(
                        "Could not find MultiFile control wrapper"
                    );
                ...
                if (!mf)
                    return !console.error(
                        "Could not find MultiFile data in wrapper"
                    );
            ...
            files: function () {
                var mf = this.MultiFile("data");
                if (!mf)
                    return !console.log("MultiFile plugin not initialized");
                return mf.files || [];
            },
            ...
            disableEmpty: function (klass) {
                ...
                $.fn.MultiFile.reEnableTimeout = window.setTimeout(
                ...
                for (var i = 0; i < methods.length; i++) {
                    ...
                    if (method)
                        (function (method) {
                            ...
                            $.fn[method] = function () {
                                $.fn.MultiFile.disableEmpty();
                                value = $.fn.MultiFile.intercepted[
                                    method
	...
	*/
        $.fn.MultiFile.options = {
            ...
            STRING: {
                ...
                toomuch: "Total Dari Keseluruh file Upload Hanya 5MB ($size)",
                toomany: "Too many files selected (max: $max)",
                toobig: "$file Lebih Besar Dari 2MB (max: $size)",
            },
